...
# ✅ Get Shopify credentials FIRST before using them
...
db = SQLAlchemy(app)  # Ensure it's bound to app here
migrate = Migrate(app, db)
bcrypt = Bcrypt(app)
login_manager = LoginManager(app)
login_manager.login_view = "login"

# ✅ Ensure Models Are Defined AFTER db is attached
class User(db.Model, UserMixin):
    ...
    ftp_host = db.Column(db.String(256), nullable=True)
    ftp_user = db.Column(db.String(256), nullable=True)
    ftp_pass = db.Column(db.String(256), nullable=True)
...
print(f"[DEBUG] Registered Routes: {app.url_map}")
...
def settings():
    if request.method == "POST":
        ...
        current_user.ftp_host = request.form.get("ftp_host")
        current_user.ftp_user = request.form.get("ftp_user")
        current_user.ftp_pass = request.form.get("ftp_pass")
...
def login():
    if request.method == 'POST':
        ...
        return "Invalid credentials!", 401  # Show error if login fails

    return render_template('home/login.html')  # Ensure the correct path
# LOGOUT ROUTE
@app.route('/logout')
def logout():
...
# ✅ Debugging Output AFTER all variables are loaded
