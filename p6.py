...
def index():
    ...
    # Initialize shopify_skus as an empty dictionary if not fetching immediately
    shopify_skus = {}
...
def download_csv_from_ftp():
    ...
    try:
        with FTP(FTP_HOST) as ftp:
            ...
            print(f"Logging into FTP server {FTP_HOST}...")
            ftp.login(FTP_USER, FTP_PASS)
            print("Logged in successfully, checking for the presence of the CSV file...")
            if CSV_FILENAME in ftp.nlst():
                print(f"Found {CSV_FILENAME}, starting download...")
        ...
        print(f"Failed to connect to FTP server or download file: {str(e)}")
        return False
...
def load_csv():
    ...
    if os.path.exists(CSV_FILENAME):
        ...
        # Map the old vendor names to the new ones
        vendor_replacements = {
        ...
        df['Brand'] = df['Brand'].replace(vendor_replacements)
        
        # Update SKU formatting if needed
...
def update_inventory_and_pricing(product_id, variant_id, inventory_item_id, quantity, suggested_retail, cost, map_price, location_id):
    try:
        ...
        inventory_response = requests.post(inventory_url, json=inventory_payload, headers={
        ...
        pricing_response = requests.put(pricing_url, json=pricing_payload, headers={
            "X-Shopify-Access-Token": ACCESS_TOKEN,
            "Content-Type": "application/json"
        })
...
def bulk_update_inventory(df, shopify_skus, location_id):
    matched_count = 0  # Counter for matched SKUs
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        ...
        for index, row in df.iterrows():
            ...
            if sku_data:
                matched_count += 1  # Increment the counter when a match is found
