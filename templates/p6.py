...
@app.route('/')
def index():
    # Fetch some initial data to display on the dashboard
    location_id = get_shopify_location_id()  # Example of integrating an existing function
    shopify_skus = fetch_all_shopify_skus() if location_id else {}

    # You could process more data here or prepare other variables to pass to the template
    return render_template('index.html', location_id=location_id, shopify_skus=shopify_skus)
